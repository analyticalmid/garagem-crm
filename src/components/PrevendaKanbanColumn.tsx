...
export const PrevendaKanbanColumn: React.FC<PrevendaKanbanColumnProps> = ({
  ...
  const colorConfig: Record<string, { dot: string }> = {
    'prevenda-novo': { dot: 'bg-[hsl(var(--prevenda-novo))]' },
    'prevenda-negociando': { dot: 'bg-[hsl(var(--prevenda-negociando))]' },
    'prevenda-analise': { dot: 'bg-[hsl(var(--prevenda-analise))]' },
    'prevenda-comprado': { dot: 'bg-[hsl(var(--prevenda-comprado))]' },
    'prevenda-standby': { dot: 'bg-[hsl(var(--prevenda-standby))]' },
  };

  const cfg = colorConfig[color] || { dot: 'bg-muted-foreground' };
      ...
      <div className="px-5 py-4 border-b border-white/[0.04]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className={`w-2 h-2 rounded-full pulse-dot ${cfg.dot}`} />
        ...
        {(provided, snapshot) => (
          ...
          >
            {leads.map((lead, index) => (
              <PrevendaLeadCard key={lead.id} lead={lead} index={index} onClick={() => onLeadClick?.(lead)} />
            ))}
